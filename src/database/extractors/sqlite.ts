  ...
  private enrichTable(db: Database, table: SQLiteTableInfo): Promise<SQLiteTableInfo> {
    return new Promise((resolve, reject) => {
      ...
      db.all(`PRAGMA table_info("${tableName}")`, (err, rows: any[]) => {
        ...
        // Get indexes
        db.all(`PRAGMA index_list("${tableName}")`, (err, indexRows: any[]) => {
          if (err) {
            reject(err);
            return;
          }
          ...
          const indexesPromises = indexRows
            ...
            .map((index) => {
              return new Promise<SQLiteIndexInfo>((resolveIdx, rejectIdx) => {
                db.all(`PRAGMA index_info("${index.name}")`, (err, colRows: any[]) => {
                  if (err) {
                    rejectIdx(err);
                    return;
                  }
                  ...
                  const isAutoIncrement =
                    index.origin === 'u' && columnsList.length === 1 && columnsList[0] === 'id';
          ...
          Promise.all(indexesPromises)
            .then((indexes) => {
              // Get foreign keys
              db.all(`PRAGMA foreign_key_list("${tableName}")`, (err, fkRows: any[]) => {
                if (err) {
                  reject(err);
                  return;
                }
