...
export function registerAnalysisCommands(program: Command): void {
  ...
  program
    ...
    .option('--route <routes>', 'Comma-separated routed contexts to prioritize (auth,db,ui)')
    .option('--level <n>', 'Escalation level override (1,2,3)')
    .option('--state', 'Include decision/hypothesis state logs in retrieval')
  ...
  program
    ...
    .option('--decision <text>', 'Record a decision in context decision log')
    .option('--hypothesis <text>', 'Record a hypothesis in context hypothesis log')
    .option('--hypothesis-status <status>', 'Hypothesis status (open,ruled-out,confirmed)', 'open')
    .option('--goal <text>', 'Record the active refactor/rewrite goal in ledger')
    .option('--non-negotiable <text>', 'Record a non-negotiable invariant in ledger')
    .option('--open-question <text>', 'Record an open question in ledger')
    .option('--failure <text>', 'Record encountered failure mode in ledger')
    .option('--resolution <text>', 'Record applied resolution in ledger')
